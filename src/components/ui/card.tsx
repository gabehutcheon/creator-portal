...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-[var(--surface-elevated)] text-[var(--text-primary)] flex flex-col gap-6 rounded-xl border border-[var(--border-default)] py-8 shadow-sm hover:border-[var(--border-hover)] transition-colors",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("font-sans text-2xl font-bold leading-tight tracking-tight text-[var(--text-primary)]", className)}
      {...props}
    />
  )
}
